...
def is_hallucination(text: str) -> bool:
    ...
    # Check if same as recent transcriptions (repeating nonsense)
    if text_lower in recent_transcriptions:
        # If we've seen this exact text in last 5 transcriptions, it's suspicious
    ...
    # Very short transcriptions are suspicious
    if len(text_lower) < 3:
        return True
...
def record_with_vad() -> Optional[List[int]]:
    ...
    # Don't listen while robot is speaking (motors active)
    if g_is_speaking:
        return None
    ...
    try:
        ...
        buffer = b''
        rms_history = []  # Track RMS to validate speech quality
        ...
        while g_running and g_is_listening and not g_is_speaking:
            ...
            if len(buffer) >= frames_per_check * 2:
                ...
                # If speech has started, record everything
                if speech_started:
                    ...
                    silence_duration = silence_frames * 0.1  # Each frame is 100ms
        ...
        if speech_frames and rms_history:
            ...
            # Real speech has high peak energy and good average energy
            if duration < MIN_SPEECH_DURATION:
                print(f"⚠️  Speech too short ({duration:.1f}s), ignoring...")
                return None
            
            # If average RMS is too low, it's probably just noise
            if avg_rms < SPEECH_START_THRESHOLD * 0.8:
                print(f"⚠️  Audio too quiet (avg: {avg_rms:.0f}), probably not speech...")
                return None
            
            # If max RMS is not much higher than average, it's constant noise (motors)
